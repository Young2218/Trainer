...
from model.vgg import VGG, VGG_Arcface
...
def train_vgg_kvasir(batch_size = 4, lr = 1e-4):
    seed_everything(17)
    experiment_root = "/home/prml/chanyoung/Trainer/_RESULT/ExtraNode/"
    ...
    model_save_path = os.path.join(experiment_root, "VGG_kvasir_base" +".pt")
    log_save_path = os.path.join(experiment_root, "VGG_kvasir_base/")
    eval_dict = {'acciracy':MulticlassAccuracy(num_classes=num_class).to(device), "f1": MulticlassF1Score(num_classes=num_class).to(device), 
                 "recall":MulticlassRecall(num_classes=num_class).to(device), "precision": MulticlassPrecision(num_classes=num_class).to(device)}
...
def train_vgg_kvasir_angular_loss(batch_size = 4, lr = 1e-4):
    seed_everything(17)
    experiment_root = "/home/prml/chanyoung/Trainer/_RESULT/ExtraNode/"
    ...
    model_save_path = os.path.join(experiment_root, "VGG_kvasir_arcface" +".pt")
    log_save_path = os.path.join(experiment_root, "VGG_kvasir__arcface/")
    eval_dict = {'acciracy':MulticlassAccuracy(num_classes=num_class).to(device), "f1": MulticlassF1Score(num_classes=num_class).to(device), 
                 "recall":MulticlassRecall(num_classes=num_class).to(device), "precision": MulticlassPrecision(num_classes=num_class).to(device)}
